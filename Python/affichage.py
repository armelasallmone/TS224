...
def periodogramme():
    bruit = np.random.normal(mean, std / rsb, len(data))
     ...
     # Apply a window function (boxcar in this case) manually
    window = get_window('boxcar', 100)
    bruit_windowed = bruit[:100] * window

    # Calculate DSP using the periodogram de daniell with window size 10 and nfft 128
    f_daniell, dsp_daniell = periodogram(bruit_windowed, fs=Fs, nfft=128)
    ...
    plt.clf()
    plt.plot(f_daniell, dsp_daniell)
...
boutonrecharger = tk.Button(fenetre, text="Recharger le signal initial", command=charger)
boutonrecharger.pack()


###  Spectre de puissance
boutonpuissance = tk.Button(fenetre, text="Spectre de puissance",command=dsp)
boutonpuissance.pack()
